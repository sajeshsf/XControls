...
using static XControls.Utilities.Helpers.StringConstants;
...
namespace XControls.Utilities.FileHandling.Spreadsheet
{
    public static class Writer
    {
        public static Task WriteToFile(this string content, string fileName,
         string? header = null, Encoding? encoding = null) => Task.Run(async () =>
         {
             using var file = encoding == null ? new StreamWriter(fileName, true) : new StreamWriter(fileName, true, encoding);
             if (header != null)
             {
                 await file.WriteAsync(header).ConfigureAwait(true);
             }
             await file.WriteAsync(content).ConfigureAwait(true);
         });
