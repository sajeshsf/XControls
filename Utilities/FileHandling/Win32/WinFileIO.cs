...
namespace Xcross.Lib.Common.Utilities.FileHandling.Win32
{
    public unsafe class WinFileIO : IDisposable
    {
        ...
        private static unsafe extern IntPtr CreateFile
        (
        ...
        [DllImport("kernel32", SetLastError = true)]
        private static unsafe extern bool CloseHandle
        (
        ...
        public void OpenForWriting(string FileName)
        {
            ...
            if (pHandle == IntPtr.Zero || (ulong)pHandle.ToInt64() == 0xffffffffffffffff) //
            {
                Win32Exception WE = new Win32Exception();
                ApplicationException AE = new ApplicationException($"WinFileIO:OpenForWriting - Could not open file ({FileName}) - {WE.Message}");
                throw AE;
            }
        }
        ...
        public int Write(int BytesToWrite)
        {
            ...
            if (!WriteFile(pHandle, pBuffer, BytesToWrite, &NumberOfBytesWritten, 0))
            {
                ...
                ApplicationException AE = new ApplicationException(
                    $"WinFileIO:Write - Error occurred writing a file . - {WE.Message}, ErrorCode - {error}");
                throw AE;
            }
